...
$ mysql -uroot mysql
Mariadb[mysql]> update user set password=password('lenapark47') where user='root';
...
MariaDB [orademo]> create user 'minuee'@'localhost' identified by 'lenapark47';
Query OK, 0 rows affected (0.00 sec)
 
-- 혹시라도 외부에서 접속하려면 localhost가 아닌 '%'로 추가 생성하여야 합니다.
MariaDB [orademo]> grant all privileges on *.* to minuee@localhost;
Query OK, 0 rows affected (0.00 sec)
...
GRANT ALL PRIVILEGES ON *.* TO 'root'@'*' IDENTIFIED BY 'lena47';
...
sudo add-apt-repository 'deb [arch=amd64,arm64,ppc64el] http://ftp.kaist.ac.kr/mariadb/repo/10.3/ubuntu bionic main'
 ...
 1991  sudo atp-get update
 1992  sudo apt-get update
 1993  sudo apt-get upgrade
 1994  sudo apt-get dist-upgrade
 1995  sudo reboot
