        ...
        <div class="row">
            <div class="col-lg-3 col-xs-6">
              ...
              <div class="small-box2 bg-purple">
                ...
                <a class="small-box-footer">To Do  현황</a>
              </div>
            </div><!-- ./col -->
            <div class="col-lg-3 col-xs-6">
              <!-- small box -->
            ...
            <div class="col-lg-3 col-xs-6 <?=($LoginSession['role'] == ROLE_ADMIN  )?'display_none':'' ?>">
              ...
              <div class="small-box bg-red">
                <div class="inner">
                  <h3><?=isset($IsReported)?'등록완료':'미진행'?></h3>
                  <p>일일리포트</p>
                </div>
                <div class="icon">
                  <i class="fa fa-newspaper-o"></i>
                </div>
                <a href="#" class="small-box-footer">&nbsp;<span class="btn btn-xs btn-default <?=isset($IsReported)?'display_none':''?>" onclick="fn_todayreport();">일일리포트 작성</span></a>
              </div>
            </div><!-- ./col -->
            <div class="col-lg-3 col-xs-6 <?=($LoginSession['role'] != ROLE_ADMIN  )?'display_none':'' ?>">
...
<script>
    ...
    var KanbanTest = new HackersKanban({
        ...
        boards: [
            {
                id: "_todo",
                title: "To Do <span class='f_r'><button class='btn btn-primary btn-xs' id='addToDo'>Todo 추가</button></span>",
                class: "info,good",
                dragTo: ["_working"],
                item: [
                ]
   ...
   ?>
    KanbanTest.addElement("_todo", {
        ...
        title: "<?=$_isemergency?><?=$_isIsReOpen?><?=$_istogether?><span class='<?=$val['Background']?>' id='Background_<?=$val['ProjectWorkIdx']?>'>[<?=$val['ProjectTitle']?>]<br /><i class='fa fa-info-circle noh_cursor btn_click_info text-light-blue ' data-idx='<?=$val['ProjectWorkIdx']?>'></i>&nbsp;&nbsp;<b><?=htmlspecialchars($val['title'])?> </b><?=$_intranetURL?><br /><span class='recently_comment'><?=$val['Comment']?></span><input type='hidden' id='ChildMode_<?=$val['ProjectWorkIdx']?>' value='<?=$val['ChildMode']?>'><input type='hidden' id='Foretime_<?=$val['ProjectWorkIdx']?>' value='<?=$val['Foretime']?>'></span>",
    ...
    KanbanTest.addElement("_working", {
        id: <?=$val['ProjectWorkIdx']?>,
        title: "<?=$_isemergency?><?=$_isIsReOpen?><?=$_istogether?><span class='<?=$val['Background']?>' id='Background_<?=$val['ProjectWorkIdx']?>'>[<?=$val['ProjectTitle']?>]<br /> <b><i class='fa fa-info-circle noh_cursor btn_click_info text-light-blue' data-idx='<?=$val['ProjectWorkIdx']?>'></i>&nbsp;&nbsp;<?=htmlspecialchars($val['title'])?></b><?=$_intranetURL?><br /><span class='recently_comment'><?=$val['Comment']?></span></span>",
        drag: function(el, source) {
            fn_checkdoing(source.parentNode.dataset.id);
            return false;
        },
...
<? } ?>
<script src="<?php echo base_url(); ?>assets/kanban/mykanban.js?v=<?=time()?>"></script>
