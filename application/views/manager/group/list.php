                    ...
                    <ul class="this_sitemap"  id="teamKanban">
                        ...
                        foreach ( $Group as $key => $val) {
                            ...
                            <li id="home" class="this_sitemap kanban-container">
                                <? if ( $val['COUNT'] > 0 ) { ?>
                                    <a  onclick="move_detph3(<?=$val['IDX']?>)"  style="<?=$_group_bg_class?>">
                                        <i class="<?=$_group_icon_class?>"></i>
                                        <?=$val['NAME']?> <small>조직번호 : <?=$val['IDX']?> 인원수 : <?=number_format($val['COUNT'])?></small>
                                    </a>
                                <? }else{?>
                                    <a onclick="move_detph3(<?=$val['IDX']?>)"  style="<?=$_group_bg_class?>">
                                        <i class="<?=$_group_icon_class?>"></i>
                                        <?=$val['NAME']?> <small>조직번호 : <?=$val['IDX']?></small>
                                    </a>
                                <? } ?>
                                <? if ( count($val['SUB']) > 0 ) {?>
...
<script>
    ...
    function move_detph3(_idx) {
        location.href = "/manager/group/view/" + _idx;
    }
