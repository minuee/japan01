...
<div class="content-wrapper clear" style="clear:both !important;">
    <section class="content-header" >
        <h1>기획본부 &gt;<?=@$GLOBAL_GROUP_NAME->NAME?></h1>
    </section>
    <div class="row">
                    ...
                    <ul class="this_sitemap"  id="teamKanban">
                        ...
                        foreach ( $Group as $key => $val) {
                            if ( $LoginSession['role'] != ROLE_EMPLOYEE) {
                                ...
                                $_group_icon_class= "cursor: pointer;";
                                $_group_bg_class= "";
                                $_is_per = false;
                            }
                            ?>
                            <li id="home" class="this_sitemap kanban-container">
                                <a onclick="move_detph4(<?=$val['IDX']?>);"  style="<?=$_group_bg_class?>"><i class="<?=$_group_icon_class?>"></i> <?=$val['NAME']?> <small>조직번호 : <?=$val['IDX']?>인원수 : <?=number_format($val['COUNT'])?></small> </a>
                                <? if ( count($val['SUB']) > 0 ) {?>
...
<script>
    ...
    function move_detph4(_idx) {
        location.href = "/manager/group/view/" + _idx +"/2";
    }
</script>