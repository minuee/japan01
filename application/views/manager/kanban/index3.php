        ...
        <div class="h_con TodoWrapper thisdroppable" id="ToDoStandbyArea" >
            <?/* if ( count($ToDoReady) > 0 ) { */?><!--
                <?/*  foreach( $ToDoReady as $tkey => $trow) {*/?>
                    <div class="kanban-item Thisdraggable nowReadyList" style="cursor: pointer !important;" id="workid_<?/*=$trow['ProjectWorkIdx']*/?>" data-id="workid_<?/*=$trow['ProjectWorkIdx']*/?>" data-idx="<?/*=$trow['ProjectWorkIdx']*/?>">
                        <?/* if ( $trow['Priority'] == 9 ) { */?>
                                        ...
                                        <header class="kanban-board-header info">
                                            <div class="kanban-title-board">
                                                ...
                                                <span class='f_r <?=($IsRegistPermission !== true || $LoginSession->userdata('role') == ROLE_ADMIN ?'display_none':'')?>'>
                                                    ...
                                                    <button class="btn btn-default btn-xs btn_each_change_seq <?=count($trow['SUB'][1])>1?'':'display_none'?>" data-idx="<?=$trow['SITE_IDX']?>">순서변경</button>
                                                </span>
                                            </div>
                                        </header>
...
<script src="<?php echo base_url(); ?>assets/js/kanban/index3.js?v=<?=time()?>"></script>
