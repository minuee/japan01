...
class Login_model extends CI_Model
{
    ...
    {
        $this->db->select('BaseTbl.userId, BaseTbl.password, BaseTbl.name,BaseTbl.nickname, BaseTbl.roleId,BaseTbl.face,BaseTbl.hackersid, Roles.role, BaseTbl.hackersidx, M.FACE_URL');
        $this->db->select('M.GROUP_IDX,M.HR_VAR_9');
        $this->db->select('G.DEPTH,G.PARENT');
        $this->db->from('tbl_users as BaseTbl');
        $this->db->join('tbl_roles as Roles','Roles.roleId = BaseTbl.roleId');
        $this->db->join('ADMIN_MEMBER as M'," BaseTbl.hackersidx = M.MEMBER_IDX  ",'LEFT');
        $this->db->join('ADMIN_MEMBER_GROUP as G'," M.GROUP_IDX = G.IDX  ",'LEFT');
